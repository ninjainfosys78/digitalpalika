...
export default function InsightsRail({ language, insights }: InsightsRailProps) {
  const content =
    language === "en"
      ...
      : { title: "इनसाइट्स", viewAll: "सबै इनसाइट्स हेर्नुहोस्", readMore: "थप पढ्नुहोस्" }
    ...
    <section id="insights" className="py-16 md:py-20 bg-black" aria-labelledby="insights-title">
      <div className="max-w-[1600px] mx-auto px-6 sm:px-8 lg:px-12">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-12 sm:mb-16 gap-4">
          <h2 id="insights-title" className="text-[32px] leading-10 font-semibold text-[#e3e3e3]">
            {content.title}
          </h2>
          <Link
            href="/blogs"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {insights.map((insight, idx) => (
            <article key={insight.url} className={`group cursor-pointer ${idx === 0 ? "md:col-span-2 md:row-span-2" : ""}`}>
              <Link
                href={insight.url}
        ...
        <div className="mt-8 sm:mt-12 text-center md:hidden">
          <Link href="/blogs" className="inline-flex items-center gap-2 text-sm transition-colors text-[#d52020] hover:text-[#782121]">
            {content.viewAll}
            <ArrowRight size={16} />
          </Link>
        </div>
