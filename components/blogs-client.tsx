...
export default function BlogsClient({ posts, initialLanguage = "en" }: BlogsClientProps) {
  const [language, setLanguage] = useState<Lang>(initialLanguage)

  const t = {
    en: { label: "Blog", heading: "Insights", breadcrumb: "Blog", readMore: "Read more" },
    ne: { label: "ब्लग", heading: "इनसाइट्स", breadcrumb: "ब्लग", readMore: "थप पढ्नुहोस्" },
  } as const
              ...
              <div className="max-w-[1200px] text-left">
                ...
                <nav aria-label="Breadcrumb" className="mt-4 text-sm text-white/80">
                  <ol className="flex items-center gap-3">
                    ...
                    <li className="font-medium tracking-wide">{c.breadcrumb}</li>
                  </ol>
                </nav>
              </div>
            </div>
              ...
              {posts.map((post) => (
                ...
                >
                  ...
                  <div className="flex-1 px-6 py-5">
                    ...
                    {post.deck && (
                      <p className="mb-6 line-clamp-3 text-sm leading-relaxed text-white/70">{post.deck}</p>
                    )}
                  </div>

                  {/* Read More */}
                  <div className="px-6 pb-6">
                    <Link href={`/blogs/${post.slug}`} className="inline-flex items-center gap-2 bg-[#E6232D] px-4 py-2 text-sm font-medium text-white">
                      {c.readMore}
                      <svg
